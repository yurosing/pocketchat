...
| оффлайн | `PM_OFFLINE` отправителю; сообщение кладётся в почтовый ящик получателя |

Отложенные сообщения хранятся на диске (`offline-mail.yml`, до 200 на игрока) и
доставляются автоматически как `PM_RECV`, как только получатель заходит на сервер
и его клиент присылает `HELLO` — до этого момента они переживают и перезапуск
сервера. Другой плагин может перехватить доставку сам — см.
`PocketChatMessageOfflineEvent` в [Plugin API](./plugin.md).
