 ...
 * {@link PmConfig#rulesAccepted} не выставлен. Без принятия окно мессенджера
 * не откроется: см. {@link PmChatClient#openMessenger}, единственную точку
 * входа, которой положено пользоваться вместо {@code new PmScreen()} напрямую.
 */
...
public class PmRulesScreen extends Screen {
    ...
    protected void init() {
        ...
        eulaBoxY = y;
        y += lineCount("pmchat.rules.eula", textW) * 10 + 10;
        eulaBoxH = y - eulaBoxY;
        y += 8;

        freedomBoxY = y;
        y += lineCount("pmchat.rules.freedom", textW) * 10 + 10;
        freedomBoxH = y - freedomBoxY;
        y += 8;

        rulesBoxY = y;
        y += 12; // заголовок «но есть базовые правила»
        y += lineCount("pmchat.rules.rule1", textW - 10) * 10;
        y += lineCount("pmchat.rules.rule2", textW - 10) * 10;
        y += 6;
    ...
    private int lineCount(String key, int maxW) {
        return Math.max(1, textRenderer.wrapLines(Text.translatable(key), maxW).size());
    }
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        drawWrapped(context, "pmchat.rules.eula", tx + 4, py + eulaBoxY + 5, textW - 8, LABEL);

        drawBox(context, freedomBoxY, freedomBoxH);
        drawWrapped(context, "pmchat.rules.freedom", tx + 4, py + freedomBoxY + 5, textW - 8, LABEL);

        drawBox(context, rulesBoxY, rulesBoxH);
        int ry = py + rulesBoxY + 5;
        context.drawText(textRenderer, Text.translatable("pmchat.rules.header"), tx + 4, ry, SUBTLE, false);
        ry += 12;
        ry = drawBullet(context, "pmchat.rules.rule1", tx + 4, ry, textW - 18);
        drawBullet(context, "pmchat.rules.rule2", tx + 4, ry, textW - 18);

        int fy = py + rulesBoxY + rulesBoxH + 10;
        drawWrapped(context, "pmchat.rules.footer", tx, fy, textW, SUBTLE);
    ...
    private void drawWrapped(DrawContext context, String key, int x, int y, int maxW, int color) {
        for (var line : textRenderer.wrapLines(Text.translatable(key), maxW)) {
            context.drawText(textRenderer, line, x, y, color, false);
            y += 10;
        }
    }

    private int drawBullet(DrawContext context, String key, int x, int y, int maxW) {
