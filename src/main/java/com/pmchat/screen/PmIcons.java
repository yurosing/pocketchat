package com.pmchat.screen;

import net.minecraft.client.gui.GuiGraphicsExtractor;

/**
 * Свои пиксельные иконки для кнопок мессенджера — не зависим от шрифта Minecraft.
 * Каждая иконка — сетка 9×9: '#' = закрашенный пиксель. Рисуется через fill()
 * и красится любым цветом (аргумент color). Так иконки всегда узнаваемы.
 */
public final class PmIcons {

    private PmIcons() {
    }

    /**
     * Рисует иконку по центру прямоугольника x0,y0,w,h цветом color (ARGB).
     * '#' — полный пиксель, '+' — 55% (сглаживание углов), '.' — 28%.
     */
    public static void draw(GuiGraphicsExtractor ctx, String[] bmp, int x0, int y0, int w, int h, int color) {
        int gw = bmp[0].length();
        int gh = bmp.length;
        int ox = x0 + (w - gw) / 2;
        int oy = y0 + (h - gh) / 2;
        int soft = withAlpha(color, 0.55f);
        int faint = withAlpha(color, 0.28f);
        for (int ry = 0; ry < gh; ry++) {
            String row = bmp[ry];
            for (int rx = 0; rx < row.length(); rx++) {
                char c = row.charAt(rx);
                int col = c == '#' ? color : c == '+' ? soft : c == '.' ? faint : 0;
                if (col != 0) {
                    ctx.fill(ox + rx, oy + ry, ox + rx + 1, oy + ry + 1, col);
                }
            }
        }
    }

    private static int withAlpha(int argb, float k) {
        int a = (int) (((argb >>> 24) & 0xFF) * k);
        return (a << 24) | (argb & 0x00FFFFFF);
    }

    // ----- Матрицы иконок (9×9) -----

    /** Фотоаппарат с кольцом-объективом — «Отправить фото». */
    public static final String[] PHOTO = {
            "         ",
            "   ##    ",
            "+#######+",
            "#+     +#",
            "#+ +#+ +#",
            "#+ # # +#",
            "#+ +#+ +#",
            "+#######+",
            "         ",
    };

    /** Микрофон — «Голосовое сообщение». */
    public static final String[] VOICE = {
            "   ###   ",
            "  +#+#+  ",
            "  #+++#  ",
            "  #+++#  ",
            "  #+++#  ",
            " +#   #+ ",
            "  +# #+  ",
            "   +#+   ",
            "  +###+  ",
    };

    /** Монета со знаком $ — «Перевести деньги». */
    public static final String[] MONEY = {
            "         ",
            "  #####  ",
            " #  #  # ",
            " # ### # ",
            " #  #  # ",
            " # ### # ",
            " #  #  # ",
            "  #####  ",
            "         ",
    };

    /** Столбчатая диаграмма — «Статистика чата». */
    public static final String[] STATS = {
            "         ",
            "       # ",
            "     # # ",
            "     # # ",
            "   # # # ",
            "   # # # ",
            " # # # # ",
            " # # # # ",
            "         ",
    };

    /** Горизонтальные полосы разной длины — «Создать опрос». */
    public static final String[] POLL = {
            "         ",
            " #####   ",
            "         ",
            " ####### ",
            "         ",
            " ###     ",
            "         ",
            " ######  ",
            "         ",
    };

    /** Смайлик в рамке — «Стикеры и гифки». */
    public static final String[] STICKERS = {
            "         ",
            " ####### ",
            " #     # ",
            " # # # # ",
            " #     # ",
            " # ### # ",
            " #     # ",
            " ####### ",
            "         ",
    };

    /** Улыбка — «Эмодзи». */
    public static final String[] EMOJI = {
            "  +###+  ",
            " #+++++# ",
            "#++#+#++#",
            "#+++++++#",
            "#+#+++#+#",
            "#++###++#",
            " #+++++# ",
            "  +###+  ",
            "         ",
    };

    /** Ползунки-слайдеры — «Настройки». */
    public static final String[] SETTINGS = {
            "         ",
            "   ##    ",
            " +#####+ ",
            "   ##    ",
            "         ",
            "     ##  ",
            " +#####+ ",
            "     ##  ",
            "         ",
    };

    /** Перечёркнутый круг — «Не беспокоить». */
    public static final String[] BELL = {
            "  #####  ",
            " #     # ",
            " ##    # ",
            "#  #    #",
            "#   #   #",
            "#    #  #",
            " #    ## ",
            " #     # ",
            "  #####  ",
    };

    /** Рамка окна — «Размер окна». */
    public static final String[] SIZE = {
            "         ",
            " ####### ",
            " #     # ",
            " #     # ",
            " #     # ",
            " #     # ",
            " ####### ",
            "         ",
            "         ",
    };

    /** Крестик — «Очистить ленту». */
    public static final String[] CLEAR = {
            "         ",
            " #     # ",
            "  #   #  ",
            "   # #   ",
            "    #    ",
            "   # #   ",
            "  #   #  ",
            " #     # ",
            "         ",
    };

    /** замочек — «Секретный чат» (шрифт игры не умеет 🔒, рисуем сами). */
    public static final String[] LOCK = {
            "  ###    ",
            " #   #   ",
            " #   #   ",
            "#######  ",
            "#  #  #  ",
            "#  #  #  ",
            "#     #  ",
            "#######  ",
            "         ",
    };

    /** открытая книга — «Документация» (кнопка-ссылка на сайт). */
    public static final String[] BOOK = {
            "         ",
            " ##   ## ",
            "#  # #  #",
            "#  # #  #",
            "#  # #  #",
            "#  # #  #",
            "#  # #  #",
            " ##   ## ",
            "         ",
    };

    /** страница документа с текстовыми строками и загнутым
     *  уголком — «Документация мода». */
    public static final String[] DOCS = {
            " #####   ",
            " #   ##  ",
            " # ## #  ",
            " #    #  ",
            " # ## #  ",
            " #    #  ",
            " # ## #  ",
            " ######  ",
            "         ",
    };

    /** телефонная трубка — «Позвонить» (Simple Voice Chat). */
    public static final String[] CALL = {
            "         ",
            " +##+    ",
            " ####    ",
            " ###+    ",
            " +##+    ",
            "   +##+  ",
            "    +###+",
            "    +####",
            "     +##+",
    };

    /** три точки — «Ещё» (свёрнутое меню редких действий). */
    public static final String[] MORE = {
            "         ",
            "   ##    ",
            "   ##    ",
            "         ",
            "   ##    ",
            "   ##    ",
            "         ",
            "   ##    ",
            "   ##    ",
    };

    /** треугольник — «Воспроизвести» (видеоплеер). */
    public static final String[] PLAY = {
            "         ",
            "  #      ",
            "  ##     ",
            "  ####   ",
            "  ###### ",
            "  ####   ",
            "  ##     ",
            "  #      ",
            "         ",
    };

    /** две палки — «Пауза» (видеоплеер). */
    public static final String[] PAUSE = {
            "         ",
            "  ##  ## ",
            "  ##  ## ",
            "  ##  ## ",
            "  ##  ## ",
            "  ##  ## ",
            "  ##  ## ",
            "  ##  ## ",
            "         ",
    };

    /** динамик с волной — «Громкость» (видеоплеер). */
    public static final String[] VOLUME = {
            "         ",
            "    #    ",
            "   ##  # ",
            " ####   #",
            " ####   #",
            " ####   #",
            "   ##  # ",
            "    #    ",
            "         ",
    };

    /** стрелка из рамки — «Открыть в браузере» (видеоплеер). */
    public static final String[] LINKOUT = {
            "         ",
            "    #####",
            "      ###",
            "     ## #",
            "    ##  #",
            " ####    ",
            " #  #    ",
            " #  #    ",
            " ####    ",
    };

    /** нижняя черта — «Свернуть» плеер в окошко. */
    public static final String[] MINIMIZE = {
            "         ",
            "         ",
            "         ",
            "         ",
            "         ",
            "         ",
            "         ",
            " ####### ",
            "         ",
    };

    /** рамка со стрелками — «Развернуть» плеер обратно на весь экран. */
    public static final String[] EXPAND = {
            "         ",
            " ##   ## ",
            " #     # ",
            "         ",
            "         ",
            "         ",
            " #     # ",
            " ##   ## ",
            "         ",
    };

    /** нота — «Музыка/плейлист». */
    public static final String[] NOTE = {
            "     ##  ",
            "    ###  ",
            "   ## #  ",
            "  ##  #  ",
            "  #   #  ",
            " ##  ##  ",
            "###  ##  ",
            " #    #  ",
            "         ",
    };

    /** предыдущий трек. */
    public static final String[] PREV = {
            "         ",
            " #    #  ",
            " #   ##  ",
            " #  ###  ",
            " # ####  ",
            " #  ###  ",
            " #   ##  ",
            " #    #  ",
            "         ",
    };

    /** следующий трек. */
    public static final String[] NEXT = {
            "         ",
            "  #    # ",
            "  ##   # ",
            "  ###  # ",
            "  #### # ",
            "  ###  # ",
            "  ##   # ",
            "  #    # ",
            "         ",
    };

    /** пиксельная мозаика — «Спойлер» (скрыть фото/видео до клика). */
    public static final String[] SPOILER = {
            "         ",
            " ## ##   ",
            " ## ##   ",
            "         ",
            " ## ##   ",
            " ## ##   ",
            "         ",
            " ## ##   ",
            " ## ##   ",
    };

    /** Видеокамера — «Стримы». */
    public static final String[] STREAM = {
            "         ",
            " ####    ",
            " #  #  # ",
            " #  # ## ",
            " #  # ## ",
            " #  # ## ",
            " #  #  # ",
            " ####    ",
            "         ",
    };

    /** Стилизованное «облачко»-геймпад — «Discord». */
    public static final String[] DISCORD = {
            "         ",
            "  #####  ",
            " #     # ",
            "#  # #  #",
            "#       #",
            "#  ###  #",
            "#  # #  #",
            " #     # ",
            "  #####  ",
    };

    /** Буква «i» в кружке — «Описание видео». */
    public static final String[] INFO = {
            "  +##+   ",
            " #    #  ",
            "#  ##  # ",
            "#  ##  # ",
            "#      # ",
            "#  ##  # ",
            "#  ##  # ",
            " #    #  ",
            "  +##+   ",
    };

    /** Игральная кость (три точки по диагонали) — «Игры» (монетка/КНБ). */
    public static final String[] DICE = {
            " ####### ",
            " #     # ",
            " # #   # ",
            " #     # ",
            " #  #  # ",
            " #     # ",
            " #   # # ",
            " #     # ",
            " ####### ",
    };

    /** Камень — «Камень» в игре камень-ножницы-бумага. */
    public static final String[] ROCK = {
            "  +###+  ",
            " ####### ",
            "#########",
            "#########",
            "#########",
            "#########",
            " ####### ",
            "  +###+  ",
            "         ",
    };

    /** Лист бумаги с линиями — «Бумага» в игре камень-ножницы-бумага. */
    public static final String[] PAPER = {
            "#########",
            "#.......#",
            "# ##### #",
            "#.......#",
            "# ##### #",
            "#.......#",
            "# ##### #",
            "#.......#",
            "#########",
    };

    /** Скрещенные лезвия — «Ножницы» в игре камень-ножницы-бумага. */
    public static final String[] SCISSORS = {
            "##     ##",
            " ##   ## ",
            "  ## ##  ",
            "   ###   ",
            "   ###   ",
            "  ## ##  ",
            " ##   ## ",
            "##     ##",
            "         ",
    };

    /** Глаз — «просмотры» поста канала (счётчик под своим постом). */
    public static final String[] EYE = {
            "         ",
            "         ",
            "  +###+  ",
            " #.....# ",
            "#..###..#",
            "#..###..#",
            " #.....# ",
            "  +###+  ",
            "         ",
    };

    /**
     * Галочка — «Верификация» / «Прочитано». Компактная 7×7, чтобы целиком
     * помещаться внутри кружка-бейджа диаметром ~9px, а не вылезать за рамку.
     */
    public static final String[] CHECK = {
            "       ",
            "      #",
            "     ##",
            "#   ## ",
            "## ##  ",
            " ###   ",
            "  #    ",
    };

    /** Стопка фото/кадров — «Медиа и файлы». */
    public static final String[] GALLERY = {
            "         ",
            "  ###### ",
            " #+    # ",
            "#+#    #.",
            "#.#  +.#.",
            "#.# ++ #.",
            "#.#+++ #.",
            "########.",
            " ........",
    };

    /** Голова робота с антенной — «Боты». */
    public static final String[] BOT = {
            "    #    ",
            "    #    ",
            "  #####  ",
            " ####### ",
            " #+#.#+# ",
            " ####### ",
            " # ### # ",
            "         ",
            "         ",
    };
}
